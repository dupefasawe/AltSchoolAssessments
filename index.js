// index.js file to handle the registration form prompts and validations

const message =
  "Hello! Welcome to the AltSchool Cloud Engineering Program.\n" +
...
(function runFormPrompts() {
  ...
  function validateAge(ageStr) {
    ...
    if (!Number.isInteger(age)) return "Age must be a number.";
  ...
  function promptUntilValid(message, validator) {
    while (true) {
      ...
      if (value === null) {
        // user pressed Cancel
        alert("Input cancelled. Please refresh the page to start over.");
        return null;
      }
